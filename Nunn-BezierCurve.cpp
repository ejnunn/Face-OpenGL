// Eric Nunn - CPSC5700 Computer Graphics
// Date: 11-19-19
// Nunn-BezierCurve.cpp - interactive curve design
...
public:
	...
	vec3 Point(float t) {
		...
		return (-pow(t, 3) + 3 * pow(t, 2) - 3 * t + 1) * p1 +
	...
	void DrawControlMesh(vec3 pointColor, vec3 meshColor, float opacity, float width) {
		...
		Disk(p1, 10, pointColor, 1);
		Line(p1, p2, width, meshColor, opacity);
		Disk(p2, 10, pointColor, 1);
		Line(p2, p3, width, meshColor, opacity);
		Disk(p3, 10, pointColor, 1);
		Line(p3, p4, width, meshColor, opacity);
		Disk(p4, 10, pointColor, 1);
	}
	vec3 *PickPoint(int x, int y, mat4 view) {
		...
		if (ScreenDistSq(x, y, p1, view) < 100) return &p1;
		if (ScreenDistSq(x, y, p2, view) < 100) return &p2;
		if (ScreenDistSq(x, y, p3, view) < 100) return &p3;
		if (ScreenDistSq(x, y, p4, view) < 100) return &p4;
		return NULL;
	}
};

Bezier	curve(vec3(-1,0,-1), vec3(-1,1,0), vec3(1,1,0), vec3(1,0,1));
