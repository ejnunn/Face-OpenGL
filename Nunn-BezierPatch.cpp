...
void SetCoeffs() {
	...
	mat4 g;
	for (int k = 0; k < 3; k++) {						// separately x, y, z
		for (int i = 0; i < 16; i++)
			g[i / 4][i % 4] = ctrlPts[i / 4][i % 4][k];	// geometry matrix
		mat4 c = m * g * m;								// coefficient matrix
		for (int i = 0; i < 16; i++)
			coeff[i / 4][i % 4][k] = c[i / 4][i % 4];	// xfer to 3D coeffs
	}
}
...
vec3 PointFromCoeffs(float s, float t) {
	...
	for (int i = 0; i < 4; i++)
		p += ta[i] * (s3 * coeff[i][0] + s2 * coeff[i][1] + s * coeff[i][2] + coeff[i][3]);
...
void SetVertices(int res, bool init = false) {
	...
	for (int i = 0; i < res; i++) {
		...
		for (int j = 0; j < res; j++) {
			...
			BezierPatch(s0, t0, &v->point, &v->normal); vPtr += 2;
			BezierPatch(s1, t0, &v->point, &v->normal); vPtr += 2;
			BezierPatch(s1, t1, &v->point, &v->normal); vPtr += 2;
			BezierPatch(s0, t1, &v->point, &v->normal); vPtr += 2;
		}
	}
	// transfer to GPU
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeBuffer, &vertices[0]);
...
void Display() {
	...
	patch.Draw(vec3(.7f, .2f, .5f), 3.5f);
	patch.DrawControlMesh(vec3(0, .4f, 0), vec3(1, 1, 0), 1, 2.5f);
	glDrawArrays(GL_QUADS, 0, 4*nQuadrilaterals); // HERE
    glFlush();
}
